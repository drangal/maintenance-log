...
export default function PlannedMLInfo() {
  const searchParams = useSearchParams()
  const cid = searchParams.get('cid')
  const supabase = createClient()
  const router = useRouter()

  const [plannedMaintenanceRecordsByCid, setPlannedMaintenanceRecordsByCid] =
    useState([])
  ...
  const deletePlannedMaintenanceRecord = async (plannedMaintenanceRecordId) => {
    try {
      const { error } = await supabase
        .from('planned_maintenances')
        .delete()
        .eq('id', plannedMaintenanceRecordId)

      if (error) throw error
      console.log('Запись успешно удалена')
      setPlannedMaintenanceRecordsByCid(
        plannedMaintenanceRecordsByCid.filter(
          (maintenanceRecordByCid) =>
            maintenanceRecordByCid.id !== plannedMaintenanceRecordId
        )
      )
    } catch (error) {
  ...
  useEffect(() => {
    const getMaintenanceRecordsByCid = async () => {
      try {
        ...
        setPlannedMaintenanceRecordsByCid(maintenance_records)
            ...
            <section className='w-full pt-3 md:pt-5 lg:pt-6'>
              <div className='container space-y-10 xl:space-y-16'>
                <div className='flex flex-col items-center justify-center space-y-4 text-center'>
                  <div className='space-y-2'>
                    ...
                    <p className='max-w-[900px] text-gray-500 md:text-xl/relaxed lg:text-base/relaxed xl:text-xl/relaxed dark:text-gray-400'>
                      Здесь вы можете запланировать работу на будущее.
                    </p>
                    <button
                      onClick={() =>
                        router.push('/protected/add-planned-ml?cid=' + cid)
                      }
                ...
                <div className='mx-auto grid max-w-5xl items-center gap-6 pb-12'>
                  {plannedMaintenanceRecordsByCid.length != 0
                    ? plannedMaintenanceRecordsByCid.map(
                        (plannedMaintenanceRecordByCid) => (
                          <div
                            key={plannedMaintenanceRecordByCid.id}
                          ...
                          >
                            <div className='flex flex-col border-b border-t transition-colors w-full caption-bottom text-sm'>
                              <div className='py-2 underline underline-offset-2'>
                                {plannedMaintenanceRecordByCid.work_types
                                  ?.name || '-'}
                              </div>
                              <div className='py-2'>
                                {plannedMaintenanceRecordByCid.description ||
                                  '-'}
                              </div>
                              <div className='flex justify-between py-2'>
                                <div>
                                  {' '}
                                  {plannedMaintenanceRecordByCid.reminder
                                    ? 'Напом. вкл.'
                                    : 'Напом. откл.'}
                                </div>
                                <div>
                                  {' '}
                                  {(plannedMaintenanceRecordByCid.planned_maintenance_mileage ||
                                    '0') + ' км'}
                                </div>
                                <div>
                                  {' '}
                                  {formatDate(
                                    plannedMaintenanceRecordByCid.created_at
                                  ) || '-'}
                                </div>
                              </div>
                              <div className='flex justify-between w-full py-2'>
                                <button
                                  ...
                                  onClick={() =>
                                    router.push(
                                      '/protected/edit-planned-ml?mlid=' +
                                        plannedMaintenanceRecordByCid.id
                                    )
                                  }
                                >
                                  <svg
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    deletePlannedMaintenanceRecord(
                                      plannedMaintenanceRecordByCid.id
                                    )
                                  }
                                >
